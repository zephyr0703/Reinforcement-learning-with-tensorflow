import numpy as np
import pandas as pd
import time

np.random.seed(2)

N_STATES = 6   # 1维世界的宽度
ACTIONS = ['left', 'right']     # 探索者的可用动作
EPSILON = 0.9   # 贪婪度 greedy
ALPHA = 0.1     # 学习率
GAMMA = 0.9    # 奖励递减值
MAX_EPISODES = 13   # 最大回合数
FRESH_TIME = 0.3    # 移动间隔时间

class QLearning(object):
    def __init__(self, n_states, actions):
        self._n_states = n_states
        self._actions = actions
        self._q_table = pd.DataFrame(
            np.zeros((n_states, len(actions))),  # q_table 全 0 初始
            columns=actions,  # columns 对应的是行为名称
        )
    
    def choose_action(self, state):
        # 在某个 state 地点, 选择行为
        state_actions = self._q_table.iloc[state, :]  # 选出这个 state 的所有 action 值
        if np.random.uniform() > EPSILON or state_actions.all() == 0:  # 非贪婪 or 或者这个 state 还没有探索过
            action = np.random.choice(self._actions)
        else:
            action = state_actions.idxmax()  # 贪婪模式
        return action
    
    def update(self, S, A, S_, R):
        is_terminated = False
        q_predict = self._q_table.loc[S, A]  # 估算的(状态-行为)值
        if S_ != 'terminal':
            q_target = R + GAMMA * self._q_table.iloc[S_, :].max()  # 实际的(状态-行为)值 (回合没结束)
        else:
            q_target = R  # 实际的(状态-行为)值 (回合结束)
            is_terminated = True  # terminate this episode
        self._q_table.loc[S, A] += ALPHA * (q_target - q_predict)  #  q_table 更新
        return is_terminated
    
    def get_q_table(self):
        return self._q_table

    
def get_env_feedback(S, A):
    # This is how agent will interact with the environment
    R = 0
    if A == 'right':  # move right
        if S == N_STATES - 2:  # terminate
            S_ = 'terminal'
            R = 1
        else:
            S_ = S + 1
    else:  # move left
        if S == 0:
            S_ = 0  # reach the wall
        else:
            S_ = S - 1
    return S_, R

def update_env(S, episode, step):
    # This is how environment be updated
    if S == 'terminal':
        interaction = 'Episode %s: total_steps = %s' % (episode+1, step)
        print('\r{}'.format(interaction), end='')
        time.sleep(2)
        print('\r                                ', end='')
    else:
        env_list = ['-'] * (N_STATES - 1) + ['T']  # '-----T' our environment
        env_list[S] = 'o'
        interaction = ''.join(env_list)
        print('\r{}'.format(interaction), end='')
        time.sleep(FRESH_TIME)

def rl():
    q_learning = QLearning(N_STATES, ACTIONS) # 初始化 q_learning
    for episode in range(MAX_EPISODES):
        S, step, is_terminated = 0, 0, False  # 回合初始化
        update_env(S, episode, step)  # 环境更新
        while not is_terminated:
            A = q_learning.choose_action(S)
            S_, R = get_env_feedback(S, A)
            is_terminated = q_learning.update(S, A, S_, R)  # 更新 q_table 表
            S = S_  # 探索者移动到下一个 state
            step += 1
            update_env(S, episode, step)  # 环境更新
    return q_learning.get_q_table()


if __name__ == '__main__':
    q_table = rl()
    print('\r\nQ-table:\n')
    print(q_table)
